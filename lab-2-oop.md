...
Думаю, вам уже доводилось задумываться :

> почему мы сравниваем строки через метод `equals`, а не оператором `==`?



Из предыдущих глав мы узнали, что есть примитивные и ссылочные типы данных.

```java
MyObjectClass obj = new MyObjectClass();	// ссылочный тип данных
int prim = 5;	// примитивный тип данных
```

Следует понимать, что тип данных относится именно к переменным `obj` и `prim`.

И так, что будет, если мы сравним два ссылочный типа данных? В Java оператор `==` , на самом деле, при сравнивании ссылок не обращает внимания на их значение. То есть этому оператору неважно какие поля есть у этого объекта, ему лишь интересно куда указывает эта ссылка.

Если мы будем сравнивать две ссылочные по оператору `==` мы буде сравнивать их адреса. Поэтому логично предположить, что если ссылки на объект указывают на одно и то же место, то сравнение вернет `true`
...
Ну, и чтобы доказать, что мы сравниваем лишь по адресам можно сделать следующее:
...
Как же решить эту проблему? Каким образом, нам нужно сравнивать объекты по их значениям, а не по расположению в памяти?

А почему бы не дать пользователю самому описать как сравнивать созданный объект  с другими? Ведь, действительно, это дает большую свободу самим программистам и разработчикам языка Java.

Тогда нам нужно, чтобы у всех объектов, которые будут созданы, присутствовал метод сравнения с другими объектами. Как это реализовать? Очень просто - добавить метод в `Object`, ведь абсолютно все классы в Java наследуются от этого суперкласса.

### Object

Все классы, которые существуют в Java - наследюутся от класса `Object` . При том, это сделано не только на уровне компиляции, но и на уровне самого JVM. 

Иными словами, вы не сможете создать класс, который не будет наследоваться от `Object`. Если интересно, то можете прочитать об этом в разделе "Hello World из байт-кода"

Такой механизм наследования от  `Object` дает разработчикам языка возможность создания методов, которые будут у всех классов, а соответственно и объектов.

Яркими примерами таких методов могут служить `equals`, `hashCode`, `toString` и еще несколько. К слову, некоторые из них имеют нативную реализацию (методы со специальным словом `native`)



Еще одним преимуществом использования наследования от `Object` может стать полиморфизм. Так как все объекты наследуются от него, то можем задать в какую-то сигнатуру метода тип Object, чтобы он мог принимать любые объекты (дважды подумайте перед использованием такого подхода!)



### Сравниваем объекты

Итак, разработчики Java предоставили нам метод `equals` 
